...
func TestResettableRuneBuffer(t *testing.T) {
	...
	t.Run("can reset multiple times", func(t *testing.T) {
		...
		var r = &resettableRuneBuffer{
			normalBuffer: bufio.NewReaderSize(strings.NewReader(input), 32),
		}

		// Throw away 2 bytes, now we should start at "cde"
		_, err := io.CopyN(io.Discard, r, 2)
		if err != nil {
			t.Errorf("unexpected copy fail: %w", err)
		}
		...
		n2, err := r.Read(b2)
		if err != nil {
			t.Errorf("unexpected read fail: %w", err)
		}
		if n2 != len(b2) {
		...
		if err != nil {
			t.Errorf("unexpected reset fail: %w", err)
		}
		...
		if err != nil {
			t.Errorf("unexpected second reset fail: %w", err)
		}
