...
func TestTokens(t *testing.T) {
	...
	for _, tt := range tokenTests {
		t.Run(tt.js, func(t *testing.T) {
			l := NewLexer(strings.NewReader(tt.js))
			i := 0
			tokens := []TokenType{}
			for {
				token, _ := l.Next()
...
func TestRegExp(t *testing.T) {
	...
	token, _ := NewLexer(strings.NewReader("")).RegExp()
	test.T(t, token, ErrorToken)
}

/*
// Test removed as we no longer use the normal lexer
func TestOffset(t *testing.T) {
	z := strings.NewReader(`var i=5;`)
	l := NewLexer(z)
	test.T(t, z.Offset(), 0)
	_, _ = l.Next()
...
/*
// Test removed because we no longer return errors of type parse.Error
func TestLexerErrors(t *testing.T) {
	l := NewLexer(strings.NewReader("@"))
	l.Next()
	test.T(t, l.Err().(*parse.Error).Message, "unexpected @")

	l = NewLexer(strings.NewReader("\x00"))
	l.Next()
	test.T(t, l.Err().(*parse.Error).Message, "unexpected 0x00")

	l = NewLexer(strings.NewReader("\x7f"))
	l.Next()
...
func ExampleNewLexer() {
	l := NewLexer(strings.NewReader("var x = 'lorem ipsum';"))
	out := ""
	for {
		tt, data := l.Next()
		if tt == ErrorToken {
