...
import (
	...
	"github.com/xarantolus/jsonextract/parsefork/buffer"
)
...
func (l *Lexer) RegExp() (TokenType, []byte) {
	...
	l.err = errors.New("unexpected EOF or newline")
	return ErrorToken, nil
}
...
func (l *Lexer) Next() (TokenType, []byte) {
	...
	default:
		if tt := l.consumeIdentifierToken(); tt != ErrorToken {
			if prevNumericLiteral {
				l.err = errors.New("unexpected identifier after number")
				return ErrorToken, nil
			}
			return tt, l.r.Shift()
		}
	...
	r, _ := l.r.PeekRune(0)
	l.err = fmt.Errorf("unexpected %s", parse.Printable(r))
...
func (l *Lexer) consumeNumericToken() TokenType {
	...
	if first == '0' {
		...
		if l.r.Peek(0) == 'x' || l.r.Peek(0) == 'X' {
			...
			l.err = errors.New("invalid hexadecimal number")
			return ErrorToken
			...
			l.err = errors.New("invalid binary number")
			return ErrorToken
			...
			l.err = errors.New("invalid octal number")
			return ErrorToken
		} else if l.r.Peek(0) == 'n' {
			l.r.Move(1)
			return BigIntToken
		} else if '0' <= l.r.Peek(0) && l.r.Peek(0) <= '9' {
			l.err = errors.New("legacy octal numbers are not supported")
			return ErrorToken
		}
	} else if first != '.' {
		for l.consumeDigit() {
	...
	if c == 'e' || c == 'E' {
		...
		if !l.consumeDigit() {
			l.err = errors.New("invalid number")
			return ErrorToken
		}
		for l.consumeDigit() {
		}
